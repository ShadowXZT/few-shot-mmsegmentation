...
@PIPELINES.register_module()
class FewShotResize(object):
    """Resize query images & masks, support images & masks.
    ...
    def _resize_img(self, results):
        ...
        if self.keep_ratio:
            ...
            img, scale_factor = mmcv.imrescale(
                results['query_img'], results['scale'], return_scale=True)

            results['query_img'] = img

            ns_imgs = []
            for s_img in results['support_imgs']:
                ns_img, scale_factor = mmcv.imrescale(
                    s_img, results['scale'], return_scale=True)
                ns_imgs.append(ns_img)

            results['support_imgs'] = ns_imgs
            ...
            ns_imgs = []
            for s_img in results['support_imgs']:
                ns_img, w_scale, h_scale = mmcv.imresize(
                    s_img, results['scale'], return_scale=True)
                ns_imgs.append(ns_img)
            results['support_imgs'] = ns_imgs
    ...
    def _resize_seg(self, results):
        ...
        if self.keep_ratio:
            query_mask = mmcv.imrescale(
                results['query_mask'], results['scale'], interpolation='nearest')
            ns_masks = []
            for s_mask in results['support_masks']:
                ns_mask = mmcv.imrescale(
                    s_mask, results['scale'], interpolation='nearest')
                ns_masks.append(ns_mask)
        else:
            query_mask = mmcv.imresize(
                results['query_mask'], results['scale'], interpolation='nearest')
            ns_masks = []
            for s_mask in results['support_masks']:
                ns_mask = mmcv.imresize(
                    s_mask, results['scale'], interpolation='nearest')
                ns_masks.append(ns_mask)
        results['query_mask'] = query_mask
        results['support_masks'] = ns_masks
...
class FewShotNormalize(object):
    ...
    def __call__(self, results):
        ...
        results['query_img'] = mmcv.imnormalize(results['query_img'],
                                                self.mean, self.std, self.to_rgb)
        s_imgs = results['support_imgs']
        ns_imgs = []
        for s_img in s_imgs:
            s_img = mmcv.imnormalize(s_img, self.mean, self.std, self.to_rgb)
            ns_imgs.append(s_img)
        results['support_imgs'] = ns_imgs
